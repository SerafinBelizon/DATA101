/* Crear Tablas Temporales para Direcciones */ 
USE ODS; 
...
CREATE TABLE TMP_DIRECCIONES_CLIENTES2 AS 
    ...
    INNER JOIN ODS.TMP_DIRECCIONES_CLIENTES DIR ON 
        CASE WHEN LENGTH(TRIM(CLIENTES.ADDRESS)) <> 0 THEN 
            UPPER(TRIM(CLIENTES.ADDRESS)) 
        ELSE 
            'DESCONOCIDO' 
        END = DIR.DE_DIRECCION 
        AND 
        CASE WHEN LENGTH(TRIM(CLIENTES.POSTAL_CODE)) <> 0 THEN 
            UPPER(TRIM(CLIENTES.POSTAL_CODE)) 
        ELSE 
            99999 
        END = DIR.DE_CP 
        AND 
        CASE WHEN LENGTH(TRIM(CLIENTES.CITY)) <> 0 THEN 
            UPPER(TRIM(CLIENTES.CITY)) 
        ELSE 
            'DESCONOCIDO' 
        END = DIR.DE_CIUDAD 
        AND 
        CASE WHEN LENGTH(TRIM(CLIENTES.STATE)) <> 0 THEN 
            UPPER(TRIM(CLIENTES.STATE)) 
        ELSE 
            'DESCONOCIDO' 
        END = DIR.DE_ESTADO 
        AND 
        CASE WHEN LENGTH(TRIM(CLIENTES.COUNTRY)) <> 0 THEN 
            UPPER(TRIM(CLIENTES.COUNTRY)) 
        ELSE 
            'DESCONOCIDO' 
        END = DIR.DE_PAIS; 
